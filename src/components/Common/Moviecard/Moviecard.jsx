...
const LazyImage = ({ src , height , width }) => {
  ...
  return (
    <div className="relative">
      {imageLoaded ? (
        <div className={`rounded-md lg:w-48 ${height?`h-${height}`:'h-64'} ${width?`w-${width}`:'w-44'}`}>
...
const Moviecard = ({ data , width , height , display ,handleClose}) => {
  return (
    <div onClick={handleClose}>
      <Link to={`/moviedetails/${data?.id}`}>
        <div className={`${width?`w-${width}`:'w-44'}  lg:w-48 cursor-pointer mt-1 transition-all hover:transition-all duration-700 hover:duration-700 hover:scale-110`} key={data.id}>
          <div>
            <LazyImage height={height} width={width} src={`${IMAGE_BASE_URL}/w1280/${data?.poster_path}`} />
          </div>
          <div className='text-gray-300 px-1'>
            <p className='truncate ... movie-title'>{data?.title}</p>
